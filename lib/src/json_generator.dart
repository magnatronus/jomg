...
class JsonObjectModelGenerator {
  ...
  Future <void> convert(String jsonString) async {
    ...
    attributeMap.forEach((key, value) {
      ...
      output.writeln("\tfinal ${value.type}${(value.cast != null) ? '<${value.cast}>' : ''}$canBeNull ${_prepName(key)};");
    });
    output.writeln(""); 
    ...
    output.writeln("\tstatic ${_upperCaseFirst(name)}Model fromJson(Map<String,dynamic> json){");
    output.writeln("\t\treturn ${_upperCaseFirst(name)}Model(");
    attributeMap.forEach((key, value) {
      if(value.type == "List" && value.cast != null){
          output.writeln("\t\t\t${_prepName(key)}: json['$key'].map<${value.cast}>((json) => ${value.cast}.fromJson(json)).toList(),");      
      } else {
        if( _isSubType(value.type)){
          output.writeln("\t\t\t${_prepName(key)}: ${value.type}.fromJson(json['$key']),");
        } else {
    ...
    if(imports.isNotEmpty){
      final currentContent = File('$outputDirectory/${name}_model.dart').readAsLinesSync();
  ...
  Map <String, JsonAttribute> _generateAttributeMap(String value) {
    ...
    jsonDecode(value).forEach((key, value) { 
      if(value.runtimeType.toString().startsWith("_InternalLinkedHashMap") || value.runtimeType.toString().startsWith("List")){
 
        // attempt to deal with a list
        if(value.runtimeType.toString().startsWith("List")){
          if(value.length > 0){
            attributes.putIfAbsent(key, () => JsonAttribute("List", cast: "${_upperCaseFirst(key)}Model"));
        ...
        if(value.runtimeType.toString().startsWith("_InternalLinkedHashMap")){
          attributes.putIfAbsent(key, () => JsonAttribute("${_upperCaseFirst(key)}Model"));
        ...
        attributes.putIfAbsent(key, () => JsonAttribute(value.runtimeType.toString()));
      }
    });
    return attributes;    
  }
